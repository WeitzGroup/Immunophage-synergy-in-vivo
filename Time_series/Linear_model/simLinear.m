%Function to simulate linearODE
...
function [ y, TB, time ] = simLinear( Ki, immune, bacteria, phage )
...
%----------------parameters for immunocompetence---------------
    ...
    [t1,y1] = ode45(@linearODE,tspan,y0,options,p);
    ...
    [t2,y2] = ode45(@linearODE,tspan2,yi,options,p);
    ...
    if currentTime < 155 % Bacterial pop died before end of simulation
        ...
        % simulating diff eq
        [t3,y3] = ode45(@linearODE,tspan3,yii,options,p);
    end
    if check == 1
        time = [t1; t2; t3];
        y = [y1; y2; y3];
