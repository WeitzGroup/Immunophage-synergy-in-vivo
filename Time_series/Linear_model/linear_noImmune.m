...
if P ~= 0
    ...
    v = [g(1) h(1)];
    h_leg = legend(v, 'phages','sensitive-bacteria','Location','eastoutside');
else
    % Legend for condition with phage treatment
    v = [h(1)];
    h_leg = legend(v,'sensitive-bacteria','Location','eastoutside');
end
axis([0,5,1e0,1e15])
legend boxoff
...
% 
% saveas(gcf, 'linear_BP', 'fig')
% saveas(gcf,'linear_BP','epsc')