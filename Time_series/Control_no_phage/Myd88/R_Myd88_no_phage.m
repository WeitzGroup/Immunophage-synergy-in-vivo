...
if P ~= 0
    ...
    v = [g(1) i(1) h(1) j(1)];
    h_leg = legend(v, 'phages','host immunity','sensitive-bacteria','resistant-bacteria','Location','eastoutside');
else
    % Legend for condition with phage treatment
    v = [i(1) h(1) j(1)];
    h_leg = legend(v,  'host immunity','sensitive-bacteria','resistant-bacteria','Location','eastoutside');
end
axis([0,72,1e0,1e15])
legend boxoff
...
% 
% saveas(gcf, 'RHM_myd88', 'fig')
% saveas(gcf,'RHM_myd88','epsc')