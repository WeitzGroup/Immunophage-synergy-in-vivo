% Code to simulate resistance phage saturation model(ODE)
% immunocompetent
% Phage added two hours after infection
% Dependencies: (1) psODE.m (2)simPS_WT.m (3) myEventsFcn.m
...
if P ~= 0
    ...
    v = [g(1) i(1) h(1)];
    h_leg = legend(v, 'phages','host immunity','sensitive-bacteria','Location','southeast');
else
    % Legend for condition with phage treatment
    v = [i(1) h(1)];
    h_leg = legend(v,  'host immunity','sensitive-bacteria','Location','southeast');
end
axis([0,72,1e0,1e15])
legend boxoff
...
% 
% 
%  saveas(gcf, 'PS_immunocompetence', 'fig')
%  saveas(gcf,'PS_immunocompetence','epsc')